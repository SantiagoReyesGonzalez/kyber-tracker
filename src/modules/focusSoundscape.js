...
class FocusSoundscape {
  ...
  stopAll(fadeDuration = 0.25) {
    ...
    if (this.activeNodes.length > 0) {
      this.activeNodes.forEach(nodeGroup => {
        try {
          if (nodeGroup.gain) {
            nodeGroup.gain.gain.setTargetAtTime(0.0001, ctx.currentTime, fadeDuration * 0.5);
          }
  ...
  playBrownNoise() {
    ...
    const bufferSize = ctx.sampleRate * 5; // 5 segundos en bucle
    ...
    trackGain.gain.exponentialRampToValueAtTime(0.55, ctx.currentTime + 0.3);
  ...
  playRainSound() {
    ...
    const trackGain = ctx.createGain();
    trackGain.gain.setValueAtTime(0.001, ctx.currentTime);
    trackGain.gain.exponentialRampToValueAtTime(0.4, ctx.currentTime + 0.35);
  ...
  playBinauralAlpha() {
    ...
    if (pannerL) pannerL.pan.setValueAtTime(-1, ctx.currentTime);
    ...
    if (pannerR) pannerR.pan.setValueAtTime(1, ctx.currentTime);
    ...
    const trackGain = ctx.createGain();
    trackGain.gain.setValueAtTime(0.001, ctx.currentTime);
    trackGain.gain.exponentialRampToValueAtTime(0.3, ctx.currentTime + 0.5);
